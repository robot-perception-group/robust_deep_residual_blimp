...
class MyPPO:
    def __init__(self, env, seed=0, model=MyModel, optimizer=Adam, steps_per_epoch=1000, rate=1000,
                 gamma=0.99, pi_lr=5e-5, v_lr=1e-4, batch_size=100, n_epochs=20, lam=0.95, max_grad_norm=0.5,
                 max_episode_length=int(2e3), clip_ratio=0.2, target_kl=np.Inf,
                 squash=False, clamp=True, output_log_sigma=False, log=False, cpu=True, rnn=False,
                 teacher=False, activation='relu', n_env=1, window_size=1, requires_grad=True, log_sigma=-1,
                 test_mode=False, display=True, group=None):
        # Choose a seed
        self.seed(seed)
        self.seed_no = seed
        self.group = group
        ...
        if teacher:
            # Add teacher_actions and teacher_weight as extra observation
            low = np.append(self.env.observation_space.low, -1 * np.ones((self.env.action_space.shape[0]+1, 1)))
            high = np.append(self.env.observation_space.high, np.ones((self.env.action_space.shape[0]+1, 1)))
            self.env.observation_space = spaces.Box(low=low, high=high, dtype=np.float32)
        if output_log_sigma:
        ...
        self.ac = model(self.env.observation_space, self.env.action_space, squash=squash, clamp=clamp,
                        ...
                        requires_grad=requires_grad, log_sigma=log_sigma).to(self.device)
        sync_params(self.ac)
        self.output_log_sigma = output_log_sigma
        self.optimizer = optimizer
        self.steps_per_epoch = int(steps_per_epoch / num_procs())
        if not test_mode:
            self.total_steps = self.env.get_attr(attr_name='total_timesteps')
            self.total_steps = self.total_steps[0]
        else:
            self.total_steps = self.env.total_timesteps
        ...
        if self.log:
            try:
                wandb.init(project="Turtle_paper", name="G" + str(self.group) + "_S" + str(self.seed_no))
    ...
    def train(self):
        ...
        o = self.env.env_method(method_name='deep_reset')
        self.global_count = 0
        o = torch.as_tensor(np.array(o)).to(self.device)

        for i in range(self.n_updates):
            for j in range(self.steps_per_epoch):
                with torch.no_grad():
                    o = torch.as_tensor(o).to(self.device)
                    if self.rnn:
                    ...
                    # Check if teacher has to intervene
                    teacher_help = False
                    self.env.env_method(method_name='teacher_intervene', intervene=teacher_help)
                    ...
                    next_o = torch.as_tensor(next_o).to(self.device)
                    # Update o
                    o = next_o
                    ...
                    if any(stop) or end_epoch:
                        ...
                        if end_epoch or any(end_episode):
                            ...
                            if any(end_episode):
                                ...
                                o_array = self.env.env_method(method_name='deep_reset',
                                                              indices=np.asarray(np.where(end_episode)).
                                                              squeeze().tolist())

                                o[np.where(end_episode), :] = torch.as_tensor(o_array).\
                                    to(self.device)
                    ...
                    if self.display and self.group is not None:
                        if self.global_count % self.rate == 0:
                            ...
                            # else:
                            print(
                                f"Progress: {self.global_count} / {self.total_steps} ( {100 * self.global_count / self.total_steps} % )\n"
                                f"Group: {self.group}\n"
                                f"Seed: {self.seed_no}\n"
                                f"Reward: {reward[0]}\n"
                                f"Observation: {o[0]}\n"
